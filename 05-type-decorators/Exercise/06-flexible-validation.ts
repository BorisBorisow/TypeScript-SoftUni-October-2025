  ...
  return function (
    ...
    descriptor.set = function (val: string) {
      if (val.length < minLength) {
        return new Error(
          "Runtime Error: name must have a min length of 3 characters"
        );
      }
...
function validateAge(min: number, max: number) {
  return function (
    ...
    descriptor.set = function (val: number) {
      if (val < min || val > max) {
        return new Error("Runtime Error: age must be between 1 and 100");
      }
  ...
  return function (
    ...
    descriptor.set = function (val: string) {
      if (!val.match(pattern)) {
        return new Error(`password needs to match ${pattern}`);
      }
      return originalSetter?.call(this, val);
    };
    return descriptor;
